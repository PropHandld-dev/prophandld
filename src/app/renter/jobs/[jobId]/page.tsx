  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      if (jobsError) {
        ...
        setConfirmedAlerts(
          jobsList.filter((j) => j.status === 'scheduled' && j.schedule_confirmed)
        )

        setUpcoming(
          jobsList
            .filter((j) => j.status === 'scheduled' && j.schedule_confirmed && j.proposed_date)
    ...
    <div className="min-h-screen bg-[#0C1A2E]">
      ...
      <main className="max-w-2xl mx-auto px-6 py-10">
        ...
        {confirmedAlerts.length > 0 && (
          <div className="bg-gradient-to-r from-[#0A7B7E]/15 to-[#12A5A9]/5 border border-[#12A5A9]/30 rounded-2xl p-5 mb-6">
            <h3 className="text-[#12A5A9] font-semibold text-sm mb-3">
              ✅ Schedule confirmed
            </h3>
            <div className="space-y-2">
              {confirmedAlerts.map((job) => (
                <Link
                  key={job.id}
                  href={`/renter/jobs/${job.id}`}
                  className="flex items-center justify-between bg-white/5 hover:bg-white/8 rounded-xl px-4 py-3 transition"
                >
                  <div>
                    ...
                    <p className="text-white/40 text-xs">
                      {job.proposed_date && new Date(job.proposed_date + 'T00:00:00').toLocaleDateString(undefined, { month: 'short', day: 'numeric' })} · {TIME_WINDOWS[job.proposed_window] || job.proposed_window}
                    </p>
                  </div>
              ...
              {upcoming.map((job) => (
                ...
                >
                  ...
                  <p className="text-[#12A5A9] text-xs mt-1">
                    {new Date(job.proposed_date + 'T00:00:00').toLocaleDateString(undefined, { weekday: 'long', month: 'short', day: 'numeric' })} · {TIME_WINDOWS[job.proposed_window] || job.proposed_window}
                  </p>
                </Link>
              ))}
            </div>
