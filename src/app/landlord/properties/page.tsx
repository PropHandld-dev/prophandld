  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (propertyError) {
      if (propertyError.code === '23505') {
        setError('This property address already exists in your portfolio.')
      } else {
    ...
    if (numUnits > 0) {
      ...
      const { error: unitsError, data: unitsData } = await supabase
        .from('units')
        .insert(units)
        .select()

      if (unitsError) {
        setError('Property created but units failed: ' + unitsError.message)
        setLoading(false)
        return
      }

      console.log('Units created:', unitsData)
    }
